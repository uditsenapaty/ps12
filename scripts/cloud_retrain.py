...
def main() -> None:
    ...
    exp = a.source + ("_pinn" if a.pinn else "")
    models = "[" + ",".join(f"'{m.strip()}'" for m in a.models.split(",")) + "]"

    print(f"[retrain] training {a.source}: {a.steps} steps{' +PINN' if a.pinn else ''} on the T4 …")
    print(studio.run(
        ...
        f"--steps {a.steps} --batch {a.batch} {pinn} --out {a.out} --device cuda 2>&1 | tail -6"))
